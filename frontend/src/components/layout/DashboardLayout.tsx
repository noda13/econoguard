...
export function DashboardLayout({ children, lastUpdated }: Props) {
  const updatedDate = lastUpdated ? new Date(lastUpdated) : null;
  const [isModalOpen, setIsModalOpen] = useState(false);
  const { isConfigured } = useUserProfile();
              ...
              {updatedDate ? (
                <>
                  ...
                  <div className="text-xs text-gray-500">
                    {updatedDate.toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })} 更新
                  </div>
                </>
              ) : (
